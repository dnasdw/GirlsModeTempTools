	...
	while ((uPos0 = sTxt.find(L"No.", uPos0)) != wstring::npos)
	{
		...
		if (sStmtNew != sStmtOld)
		{
			...
			do
			{
				...
				while (uPosOld0 != wstring::npos && (StartWith(sStmtOld, L"<U67", uPosOld0) || StartWith(sStmtOld, L"<U6A", uPosOld0)))
				{
					uPosOld0++;
					uPosOld0 = sStmtOld.find_first_of(L"<{", uPosOld0);
				}
				uPosNew0 = sStmtNew.find_first_of(L"<{", uPosNew0);
				while (uPosNew0 != wstring::npos && (StartWith(sStmtNew, L"<U67", uPosNew0) || StartWith(sStmtNew, L"<U6A", uPosNew0)))
				{
					uPosNew0++;
					uPosNew0 = sStmtNew.find_first_of(L"<{", uPosNew0);
				}
